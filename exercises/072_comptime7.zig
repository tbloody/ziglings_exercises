...
pub fn main() void {
    ...
    inline while (i < instructions.len) : (i += 3) {
        ...
        const digit2 = comptime (instructions[i + 1]);

        //TODO: SVT coerce into char? let's see what happens if I leave it out
        print("digit: {s} {u}\n", .{ @typeName(@TypeOf(digit)), digit });
        print("digit2: {s} {u}\n", .{ @typeName(@TypeOf(digit2)), digit2 });
